"""
Signal Visualiser Tool - Control Menu Module

This module provides a PyQt5-based GUI for audio analysis with various methods including:
...
Also it includes various helper methods
...
from PyQt5.QtWidgets import (
    QWidget, QDialog, QLabel, QPushButton, QLineEdit, QRadioButton, 
    QCheckBox, QComboBox, QGridLayout, QGroupBox, QVBoxLayout,
    QFileDialog, QDoubleSpinBox, QSpinBox, QMessageBox, QSlider, QHBoxLayout
)
...
class ControlMenu(QDialog):
    """
    Main control menu dialog for audio analysis operations.
    
    This class provides a comprehensive interface for analyzing audio files with
    various signal processing techniques and visualization methods.
    """
    
    def __init__(self, name, fs, audio, duration, controller):
        """
        Initialize the ControlMenu dialog.
        
        Args:
            name (str): Name of the audio file
            fs (int): Sampling frequency of the audio
            audio (numpy.array): Audio signal data
            duration (float): Duration of the audio in seconds
            controller: Reference to the main application controller
        """
        super().__init__(None)
        
        # Extract base name from filename (remove any brackets or special formatting)
    ...
    """Set up the user interface with all controls and layouts."""
    def setupUI(self):
    ...
    """Update the global font setting for plots."""
    ...
    """Update all open plot windows with current selection information."""
    ...
    """Create spectrogram analysis control group."""
    ...
    """Save audio data to Excel/CSV file."""
    ...
    # Audio playback and control methods

    """Update the playback position for real-time visualization."""
    ...
    """Create pitch analysis control group."""
    ...
    def create_filter_group(self, layout):
        ...
        # Connect radio buttons to UI updates
    ...
    """Create Short-Time Energy control group."""
    ...
    def update_ui_state(self, method):
        # Define mode flags for different analysis methods
    ...
    """Get frequency bounds from UI inputs."""
    ...
    """Main plotting method that routes to specific plot functions based on selection."""
    ...
    """Plot Fourier Transform of the audio signal."""
    def plot_ft(self):
        # Get current font size (default to 12 if not set)
        fontsize = getattr(self, 'current_font_size', 12)
    ...
    """Move analysis window on left SINGLE click (without dragging)."""
    ...
    def plot_ste(self):
        """Plot Short-Time Energy analysis."""
        # Get current font size (default to 12 if not set)
        fontsize = getattr(self, 'current_font_size', 12)
    ...
    def plot_spectral_centroid(self):
        """Plot spectral centroid analysis."""
        # Get current font size (default to 12 if not set)
        fontsize = getattr(self, 'current_font_size', 12)
    ...
    def calculate_sc(self, segment):
        """Calculate spectral centroid for a given audio segment."""
    ...
    def plot_filtering(self):
        """Main filtering analysis method."""
        filter_type = self.filter_type.currentText()
        percentage = float(self.percentage.text())
    ...
    # HELPER METHODS FOR PLOTTING AND INTERACTION

    """Create a span selector for interactive audio selection and playback."""
    ...
    def show_plot_window(self, figure, waveform_ax=None, audio_signal=None, create_selector=True):
        ...
        plot_dialog = QDialog()
        #plot_dialog.setParent(None)
        #plot_dialog.setWindowFlags(Qt.Window)
        plot_dialog.setWindowTitle(plot_title)
        plot_dialog.setAttribute(Qt.WA_DeleteOnClose)
        plot_dialog.plot_id = id(plot_dialog)
    ...
    """Get window function based on selected window type."""
